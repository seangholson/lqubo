...
for i in range(len(tsp_domain)):
    ...
    for trial in range(100):
        ...
        tsp_ans.append(objective_function(perm))
...
plt.errorbar(x=tsp_domain_str, y=data['avg'], yerr=data['sd'])
plt.scatter(x=tsp_domain_str, y=data['avg'], label="Average TSP answer")
plt.scatter(x=tsp_domain_str, y=data['optimal ans'], label="Optimal TSP answer")
plt.scatter(x=tsp_domain_str, y=avg_obj_vals, label="LQUBO Avg Answer")
